...
/**
 * Server-Side Role Verification for Batch Status Transitions
 * - 'ready': Admin, Quality Inspector, and Production Lead all have authority
 * - 'packaged': Admin and Quality Inspector have authority
 */
async function verifyBatchStatusRole(
  farmId: string,
  uid: string,
  targetStatus: 'ready' | 'packaged',
  token?: string
): Promise<{ authorized: boolean; roleLabel?: string }> {
  if (!farmId || !uid) return { authorized: false };

  // Development/Preview mode check
  if (token === 'preview-token' || uid.startsWith('preview-') || uid.startsWith('dev-')) {
    return { authorized: true, roleLabel: 'admin' };
  }

  if (token) {
    const payload = parseJwtPayload(token);
    if (!payload || (payload.sub !== uid && payload.user_id !== uid)) {
      console.warn('verifyBatchStatusRole: Token sub does not match passed uid');
      return { authorized: false };
    }
    const nowSec = Math.floor(Date.now() / 1000);
    if (payload.exp && payload.exp < nowSec) {
      console.warn('verifyBatchStatusRole: Token expired');
      return { authorized: false };
    }

    try {
      // 1. Farm ownership check
      const farmRes = await fetch(`${FIRESTORE_BASE_URL}/farms/${farmId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (farmRes.ok) {
        const farmDoc = convertFirestoreDoc(await farmRes.json());
        if (farmDoc?.ownerUid === uid) {
          return { authorized: true, roleLabel: 'owner' };
        }
      }
      ...
      if (memberRes.ok) {
        const memberDoc = convertFirestoreDoc(await memberRes.json());
        const permissionTier = memberDoc?.permissionTier;
        const roleLabel = (memberDoc?.roleLabel || '').toLowerCase();

        if (permissionTier === 'admin') {
          return { authorized: true, roleLabel: memberDoc?.roleLabel || 'admin' };
        }

        // For targetStatus === 'ready':
        // Quality Inspector, Production Lead, and Admin all have authorization!
        if (targetStatus === 'ready') {
          const isQC =
            roleLabel.includes('quality') ||
            roleLabel.includes('inspector') ||
            roleLabel.includes('qc');
          const isLead =
            roleLabel.includes('production') ||
            roleLabel.includes('lead');
          if (isQC || isLead) {
            return { authorized: true, roleLabel: memberDoc?.roleLabel };
          }
        }

        // For targetStatus === 'packaged':
      ...
      console.warn('verifyBatchStatusRole REST lookup warning:', err?.message);
      return { authorized: false };
    }
  }
...
app.post('/api/batches/:batchId/status', async (req, res) => {
  try {
    ...
    // Authoritative Server-Side Role Verification
    // Admin, Quality Inspector, and Production Lead all have authority to mark batch Ready
    const { authorized } = await verifyBatchStatusRole(farmId, uid, status, token);
    if (!authorized) {
      return res.status(403).json({
        error:
          status === 'ready'
            ? 'Forbidden: Marking a batch as Ready requires Quality Inspector, Production Lead, or Admin authority.'
            : 'Forbidden: Advancing batch status to Packaged requires Admin or Quality Inspector authority.',
      });
    }
