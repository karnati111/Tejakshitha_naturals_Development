...
export async function getHarvestLogs(
  ...
  try {
    ...
    console.error(`Failed to list harvest logs for farm ${farmId}:`, err);
    return [];
  }
}
...
export async function updateHarvestLog(
  ...
  if (farmId.startsWith('preview-')) {
    ...
    if (idx !== -1) {
      ...
      if (data.qcStatus !== undefined) logs[idx].qcStatus = data.qcStatus;
      if (data.qcInspection !== undefined) logs[idx].qcInspection = data.qcInspection;
      localStorage.setItem(`preview_logs_${farmId}`, JSON.stringify(logs));
      return logs[idx];
    }
  }
  ...
  try {
    ...
    if (data.qcStatus !== undefined) updatePayload.qcStatus = data.qcStatus;
    if (data.qcInspection !== undefined) updatePayload.qcInspection = data.qcInspection;
    ...
    return {
      ...
      qcStatus: updatePayload.qcStatus ?? existingData.qcStatus,
      qcInspection: updatePayload.qcInspection ?? existingData.qcInspection,
    };
  } catch (err) {
...
export async function inspectRawMaterialIntake(
  farmId: string,
  logId: string,
  inspection: RawMaterialQcInspection
): Promise<HarvestLog> {
  const qcStatus = inspection.status;

  if (farmId.startsWith('preview-')) {
    const logs = await getHarvestLogs(farmId);
    const idx = logs.findIndex((l) => l.id === logId);
    if (idx !== -1) {
      logs[idx].qcStatus = qcStatus;
      logs[idx].qcInspection = {
        ...inspection,
...
- Test leaf shatter readiness and prepare for hermetic sealing.`,
  ...
  try {
    ...
    // Sort client-side by createdAt desc safely
    ...
    console.error(`Error loading batches for facility ${farmId}:`, err);
    return [];
  }
}
    ...
    const defaultPackages: PackagingRecord[] = [
      {
        ...
        qcInspection: {
          id: 'insp-pkg-01',
          inspectedByUid: 'preview-admin-bharath',
          inspectedByName: 'Bharath (Facility Director)',
          inspectedByRole: 'Quality Controller',
          inspectedAt: new Date(Date.now() - 3600000 * 19).toISOString(),
          status: 'qc_approved',
          sampleWeightGrams: 100.8,
          weightCompliant: true,
          sealIntegrityPassed: true,
          labelAccuracyPassed: true,
          packagingCleanlinessPassed: true,
          aromaMoistureBarrierPassed: true,
          notes: 'Full airtight seal verified. Net weight 100.8g within ±1% margin. Certified for immediate dispatch.',
        },
      },
      {
        ...
        qcInspection: {
          ...
          inspectedByName: 'Bharath (Facility Director)',
          inspectedByRole: 'Quality Controller',
          inspectedAt: new Date(Date.now() - 3600000 * 17).toISOString(),
          status: 'qc_approved',
          sampleWeightGrams: 251.2,
          weightCompliant: true,
          sealIntegrityPassed: true,
          labelAccuracyPassed: true,
          packagingCleanlinessPassed: true,
          aromaMoistureBarrierPassed: true,
          notes: 'Lot label barcode verified. Pouch integrity pristine.',
        },
      },
      {
      ...
      },
      {
        ...
        qcInspection: {
          ...
          inspectedByName: 'Bharath (Facility Director)',
          inspectedByRole: 'Quality Controller',
          inspectedAt: new Date(Date.now() - 3600000 * 11).toISOString(),
          status: 'qc_approved',
          sampleWeightGrams: 200.5,
          weightCompliant: true,
          sealIntegrityPassed: true,
          labelAccuracyPassed: true,
          packagingCleanlinessPassed: true,
          aromaMoistureBarrierPassed: true,
          notes: 'Aroma evaluation passed. Heat crimp verified.',
        },
      },
    ];
...
export async function addPackagingRecord(
  ...
  if (farmId.startsWith('preview-')) {
    ...
    const newRecord: PackagingRecord = {
      ...
      qcStatus: data.qcStatus || 'pending_qc',
      createdAt: new Date().toISOString(),
    };
