...
import { ProductionBatch, FarmMember, ProgressReading, canUpdateQuality } from '../types';
import {
  Thermometer,
  Droplets,
  Clock,
...
export const QualityControlTimeline: React.FC<QualityControlTimelineProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* QC Worker Quick Logger Form (Quality Inspector & Admin Only) */}
      {canUpdateQuality(member.roleLabel, member.permissionTier) ? (
        <form
        ...
        /* Read-only audit notice for Production Lead / non-QC roles */
        <div className="p-3.5 bg-amber-50/70 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-amber-800 dark:text-amber-300">
          <div className="flex items-center space-x-2">
            <Info className="w-4 h-4 text-amber-500 shrink-0" />
            <span>
              <strong>Quality Inspection Read-Only Mode:</strong> Certified temperature checkpoints and sensory inspection updates are restricted to Quality Inspectors and Admins. Production Leads can inspect verified readings and historical timelines above.
            </span>
          </div>
          <span className="px-2 py-0.5 rounded bg-amber-200 dark:bg-amber-900 text-amber-800 dark:text-amber-200 font-bold text-[10px] uppercase self-start sm:self-auto shrink-0">
            Read-Only QC Audit
          </span>
        </div>
      )}
