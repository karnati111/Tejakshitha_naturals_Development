...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-900 border-b border-slate-800 text-slate-100 shadow-md w-full max-w-full overflow-hidden">
      <div className="max-w-7xl w-full mx-auto px-2.5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-1.5 sm:gap-4 w-full min-w-0">
          {/* Brand & Navigation Slider Trigger */}
          <div className="flex items-center space-x-1.5 sm:space-x-3 min-w-0 shrink">
            {onToggleNavSlider && (
              <button
                onClick={onToggleNavSlider}
                id="btn-header-hamburger-menu"
                className="md:hidden p-1.5 sm:p-2 rounded-xl bg-slate-800 hover:bg-slate-750 text-slate-300 hover:text-white border border-slate-700 transition cursor-pointer flex items-center justify-center shrink-0 min-h-[36px] min-w-[36px]"
            ...
            <div className="hidden xs:flex w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-emerald-500/20 border border-emerald-500/40 items-center justify-center text-emerald-400 shadow-inner shrink-0">
              <Factory className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>

            <div className="min-w-0">
              <div className="flex items-center space-x-1.5 sm:space-x-2">
                <span className="font-bold text-xs sm:text-base lg:text-lg tracking-tight text-white truncate max-w-[110px] xs:max-w-[160px] sm:max-w-none">
                  <span className="sm:hidden">Smart Harvest</span>
                  <span className="hidden sm:inline">Smart Harvest &amp; Processing Tracker</span>
                </span>
                <span className="hidden md:inline-block px-2 py-0.5 text-[10px] uppercase font-semibold tracking-wider rounded bg-emerald-950 text-emerald-300 border border-emerald-800 shrink-0">
                  Gemini 3.6 Flash
                </span>
              </div>
          ...
          <div className="flex items-center space-x-1 sm:space-x-2.5 shrink-0">
            ...
            {activeFarm && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-1 sm:py-1.5 bg-slate-800 hover:bg-slate-750 border border-slate-700 rounded-lg text-xs sm:text-sm font-medium transition text-slate-200"
            ...
            <button
              ...
              className="p-1.5 sm:p-2 text-slate-400 hover:text-white bg-slate-800/80 hover:bg-slate-800 border border-slate-700 rounded-lg transition shrink-0"
              title="Refresh Data"
            >
              <RefreshCw className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isRefreshing ? 'animate-spin text-emerald-400' : ''}`} />
            </button>

            {/* Test Walkthrough Button */}
            <button
              onClick={onOpenWalkthrough}
              className="hidden sm:flex items-center space-x-1.5 px-2.5 py-1.5 bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-slate-600 rounded-lg text-xs font-medium text-slate-300 transition shrink-0"
            ...
            {/* Active Role Badges */}
            {activeMember && (
              <div className="hidden sm:flex items-center space-x-1.5 px-2.5 py-1 rounded-lg bg-slate-800/90 border border-slate-700 text-xs shrink-0">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 pl-1 sm:pl-2 border-l border-slate-800 shrink-0">
              <div
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-200 uppercase overflow-hidden border border-slate-600 shrink-0"
              ...
              <button
                onClick={() => logoutUser()}
                className="p-1.5 sm:p-2 text-slate-400 hover:text-rose-400 hover:bg-slate-800 rounded-lg transition shrink-0"
                title="Sign Out"
              >
