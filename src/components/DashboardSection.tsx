...
export const DashboardSection: React.FC<DashboardSectionProps> = ({
  ...
  const [qcBatchFilter, setQcBatchFilter] = useState<'all' | 'running' | 'ready' | 'packaged'>('all');

  // Filter batches displayed on the QC inspection board
  const qcFilteredBatches = useMemo(() => {
    if (qcBatchFilter === 'running') {
      return batches.filter((b) => b.status === 'processing' || b.status === 'harvested');
    }
    if (qcBatchFilter === 'ready') {
      return batches.filter((b) => b.status === 'ready');
    }
    if (qcBatchFilter === 'packaged') {
      return batches.filter((b) => b.status === 'packaged');
    }
    return batches;
  }, [batches, qcBatchFilter]);
  ...
  if (isQC) {
    return (
      <div id="quality-inspector-dashboard" className="space-y-6">
        ...
        {/* Active Production Batches Quality Oversight Board */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white flex items-center space-x-2">
                ...
                <span>Production Batches Quality Inspection ({qcFilteredBatches.length})</span>
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Inspect drying/processing chambers, verify thermal thresholds, and commit temperature and condition checkpoints.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button
                onClick={() => setQcBatchFilter('running')}
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition ${
                  qcBatchFilter === 'running'
                    ? 'bg-blue-600 text-white shadow-xs'
                    : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
                }`}
              >
                In Chamber ({runningBatches.length})
              </button>
              <button
                onClick={() => setQcBatchFilter('ready')}
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition ${
                  qcBatchFilter === 'ready'
              ...
              >
                Ready for Packaging ({readyBatches.length})
              </button>
              <button
                onClick={() => setQcBatchFilter('packaged')}
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition ${
                  qcBatchFilter === 'packaged'
                    ? 'bg-purple-600 text-white shadow-xs'
                    : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
                }`}
              >
                Packaged Lots ({batches.filter((b) => b.status === 'packaged').length})
              </button>
            </div>
          </div>

          {qcFilteredBatches.length === 0 ? (
            <div className="text-center py-12 text-slate-400 space-y-3">
              <CheckCircle2 className="w-12 h-12 mx-auto text-blue-500/60" />
              <div className="font-semibold text-sm text-slate-700 dark:text-slate-300">
                {qcBatchFilter === 'all'
                  ? 'No production batches created yet in this facility'
                  : `No production batches currently match the "${qcBatchFilter}" filter`}
              </div>
              <p className="text-xs max-w-md mx-auto">
                {qcBatchFilter === 'all'
                  ? 'When the Production Lead or Facility Admin initiates a production batch, it will immediately appear here for your Quality Control checks.'
                  : 'Try selecting "All Batches" above or explore the Batches tab to review historical records.'}
              </p>
              <div className="flex items-center justify-center gap-2 pt-1">
                {qcBatchFilter !== 'all' && (
                  <button
                    onClick={() => setQcBatchFilter('all')}
                    className="px-4 py-2 bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 rounded-xl text-xs font-semibold transition"
                  >
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      <button
                        ...
                        className="flex-1 min-w-[120px] px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold shadow-xs transition flex items-center justify-center space-x-1.5"
                      >
                      ...
                      {batch.status === 'ready' && (
                        <button
                          onClick={() => {
                            onNavigateTab('packaging');
                          }}
                          className="px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-bold shadow-xs transition flex items-center space-x-1"
                        >
                      ...
                      {batch.status === 'packaged' && (
                        <button
                          ...
                          className="px-3 py-2 bg-purple-100 dark:bg-purple-950/60 hover:bg-purple-200 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 rounded-xl text-xs font-semibold transition flex items-center space-x-1"
                        >
                          <PackageCheck className="w-3.5 h-3.5" />
                          <span>Packaged Lots</span>
                        </button>
                      )}
  ...
  return (
    <div className="space-y-5 sm:space-y-6 w-full min-w-0 max-w-full">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-850 to-emerald-950 text-white p-4 sm:p-6 rounded-2xl border border-slate-800 shadow-sm w-full min-w-0 max-w-full overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full min-w-0">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-1 min-w-0">
              <Sparkles className="w-4 h-4 shrink-0" />
              <span className="truncate">Admin Operations Center</span>
              <span>•</span>
              <span className="truncate">{farm.name}</span>
              <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30 text-[10px] shrink-0">
                ADMIN AUTHORIZED
              </span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 w-full md:w-auto shrink-0">
            <button
              ...
              className="flex items-center space-x-1 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-xl text-xs font-semibold backdrop-blur-sm transition shrink-0"
            ...
            <button
              onClick={() => onNavigateTab('batches')}
              className="flex items-center space-x-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold shadow transition shrink-0"
            >
              <Boxes className="w-3.5 h-3.5" />
              <span>Batches</span>
            </button>
            <button
              onClick={() => onNavigateTab('packaging')}
              className="flex items-center space-x-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-semibold shadow transition shrink-0"
            ...
            <button
              onClick={() => onNavigateTab('dispatch')}
              className="flex items-center space-x-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-semibold shadow transition shrink-0"
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-xl text-xs font-semibold backdrop-blur-sm transition shrink-0"
            >
      ...
      {feedback && (
        <div
          className={`p-3.5 sm:p-4 rounded-xl text-xs flex items-center justify-between shadow-xs w-full min-w-0 ${
            feedback.type === 'success'
              ? 'bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-900 dark:text-emerald-200'
              : 'bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800 text-rose-900 dark:text-rose-200'
          }`}
        >
          ...
          <button
            onClick={() => setFeedback(null)}
            className="text-[11px] underline opacity-80 hover:opacity-100 shrink-0 ml-2"
          >
            Dismiss
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 w-full min-w-0">
        ...
        <div
          ...
          className={`p-4 rounded-xl border shadow-xs transition cursor-pointer group min-w-0 overflow-hidden ${
        ...
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-medium">
            <span className={readyBatches.length > 0 ? 'text-emerald-800 dark:text-emerald-300 font-bold truncate' : 'truncate'}>
          ...
          <div className="mt-2 flex items-center justify-between text-[11px] gap-1">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 group-hover:translate-x-0.5 transition-transform flex items-center shrink-0">
        ...
        <div
          ...
          className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs hover:border-blue-300 dark:hover:border-blue-800 transition cursor-pointer group min-w-0 overflow-hidden"
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-medium">
            <span className="truncate">In Chamber Processing</span>
          ...
          <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 gap-1">
            <span className="truncate">
              {harvestedBatches.length > 0 ? `+${harvestedBatches.length} staged` : 'Under environmental control'}
            </span>
            <span className="text-blue-600 dark:text-blue-400 group-hover:translate-x-0.5 transition-transform flex items-center shrink-0">
        ...
        <div
          ...
          className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs hover:border-purple-300 dark:hover:border-purple-800 transition cursor-pointer group min-w-0 overflow-hidden"
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-medium">
            <span className="truncate">Packaged &amp; Ready</span>
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            ...
            <span className="text-xs text-slate-500 truncate">packets avail</span>
          </div>
          <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 gap-1">
            <span className="truncate">
              {totalPackedUnits.toLocaleString()} total packed
            </span>
            <span className="text-purple-600 dark:text-purple-400 group-hover:translate-x-0.5 transition-transform flex items-center shrink-0">
        ...
        <div
          ...
          onClick={() => onNavigateTab('dispatch')}
          className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs hover:border-sky-300 dark:hover:border-sky-800 transition cursor-pointer group min-w-0 overflow-hidden"
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-medium">
            <span className="text-sky-700 dark:text-sky-300 font-bold truncate">Dispatched Count</span>
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            ...
            <span className="text-xs text-slate-500 truncate">dispatches logged</span>
          </div>
          <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 gap-1">
            <span className="truncate">
              {totalUnitsDispatched.toLocaleString()} units ({totalKgDispatched} kg)
            </span>
            <span className="text-sky-600 dark:text-sky-400 group-hover:translate-x-0.5 transition-transform flex items-center shrink-0">
        ...
        <div
          onClick={() => onNavigateTab('intake')}
          className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs hover:border-emerald-300 dark:hover:border-emerald-800 transition cursor-pointer group min-w-0 overflow-hidden"
        >
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 text-xs font-medium">
            <span className="truncate">Raw Material Stock</span>
          ...
          <div className="mt-2 flex items-baseline space-x-2">
            ...
            <span className="text-xs text-slate-500 truncate">
              / {totalIntakeQuantity.toLocaleString()} units
            </span>
          </div>
          <div className="mt-2">
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-2 overflow-x-auto no-scrollbar w-full min-w-0">
        <button
          onClick={() => setAdminActiveSubView('overview')}
          className={`px-3.5 sm:px-4 py-2 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 shrink-0 ${
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/70 text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700 font-semibold">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            <div className="relative w-full sm:w-auto">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none w-full sm:w-44"
              />
            </div>

            {/* Status Filter Pills */}
            <div className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg text-[11px] overflow-x-auto no-scrollbar">
              {(['all', 'processing', 'ready', 'packaged', 'harvested'] as const).map((status) => (
                <button
                  key={status}
                  onClick={() => setBatchStatusFilter(status)}
                  className={`px-2.5 py-1 rounded-md font-medium capitalize transition shrink-0 ${
